...
class main_window(customtkinter.CTk):
    def __init__(self):
        ...
        # load yolo model
        self.model = torch.hub.load('ultralytics/yolov8', 'yolov8x')
    ...
    def CapVideo1(self, IsOpenVideo):
        if IsOpenVideo is True:
            ...
            n = 1
            while True:
                if f'{file_name}{n}.mp4' in files:
                    n += 1
                    continue
                else:
                    new_filename = f'{file_name}{n}.mp4'
                    break
    ...
    def update1(self):
        ...
        if ret == True:
            ...
            for detection in detections:
                ...
                if class_idx in [2, 7]:  # 只取汽車和卡車
                    ...
                    if (is_in_list(self.list_n, center)):
                        if (center_x / center_y < 2.8):
                            cv2.circle(frame, (center_x, center_y), 8, (0, 255, 255), -1)
                            self.n += 1

                    if (is_in_list(self.list_e, center)):
                        if(center_x/center_y<2.5):
                            cv2.circle(frame, (center_x, center_y), 8, (0, 255, 255), -1)
                            self.e += 1
            ...
            if(sorted_sort[0] == self.n):
                ...
                self.label_Congestion_W.configure(text=str("普通"))
                self.label_Congestion_E.configure(text=str("普通"))
                ...
                self.label_Congestion_W.configure(text=str("較高"))
                self.label_Congestion_E.configure(text=str("較低"))
                self.label_Congestion_N.configure(text=str("普通"))
                self.label_Congestion_S.configure(text=str("普通"))
            else:
                self.label_Congestion_W.configure(text=str("較低"))
                self.label_Congestion_E.configure(text=str("較高"))
                self.label_Congestion_N.configure(text=str("普通"))
                self.label_Congestion_S.configure(text=str("普通"))
